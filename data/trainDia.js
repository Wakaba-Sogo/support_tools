...
function displayData() {
    if (trainList.length) {
        ...
        trainList.forEach(t => {
            let displayE;
            if (Number(t.del)) displayE = `<div id="${t.num}"><span>${t.htd}</span> <span class="delay">+${t.del}</span>　<span>${t.trn}</span> <span>${t.st}</span> <span>${t.car}両編成</span><br>現在位置 <span>${t.pos}</span><br><span>予想混雑度</span> <span>${t.cr != "-1" ? t.cr : "不明"}</span></div><br>`;
            else displayE = `<div id="${t.num}"><span>${t.htd}</span>　<span>${t.trn}</span> <span>${t.st}</span> <span>${t.car}両編成</span><br>現在位置 <span>${t.pos}</span><br><span>予想混雑度</span> <span>${t.cr != "-1" ? t.cr : "不明"}</span></div><br>`;
            /*　例
